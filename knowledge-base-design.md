...
SBONの日本語説明・レビュー観点・リスク判定を、単純な辞書から知識ベースへ発展させる。

説明辞書を増やすだけでは、パッケージ名の揺れ、エコシステム差、根拠情報、更新履歴、組織ごとの判断を扱いきれない。SBONでは、OSSコンポーネントを「名前」ではなく「正規化されたソフトウェア部品」として扱い、説明・リスク・参照情報を紐づける。
...
同じOSSを指す可能性がある名前や識別子を管理する。

| field | type | description |
| --- | --- | --- |
| id | string | 内部ID |
| package_id | string | `packages.id` |
| alias_type | string | `name`, `purl`, `cpe`, `distro`, `spdx`, `regex` |
| value | string | 別名またはパターン |
...
| override_type | string | category, risk, note, approved_version |
| value | string | 補正値 |
| reason | string | 理由 |
...
3. 照合ロジックを `name regex` から `alias based matching` に置き換える
4. テストで一致精度とリスク判定を固定する
